 ...
 */
class GenericCommandClientServerPoolState<T>
{
	/**
	 * Init the state to open
	 */
	GenericCommandClientServerPoolState()
	{
		fReopener = null;
		fIsOpen = true;
	}
	 ...
	 */
	synchronized InterruptibleFutureTask<GenericCommandClientServer<T>> close()
	{
		InterruptibleFutureTask<GenericCommandClientServer<T>> 	localReopener = fReopener;
	 ...
	 * @return true/false
	 */
	synchronized boolean checkSetReopener(InterruptibleFutureTask<GenericCommandClientServer<T>> newReopener)
	{
	 ...
	 */
	synchronized InterruptibleFutureTask<GenericCommandClientServer<T>> getAndCheckDoneReopener(AtomicReference<Boolean> isOpen)
	{
		isOpen.set(fIsOpen);
	...
	private InterruptibleFutureTask<GenericCommandClientServer<T>> 		fReopener;
