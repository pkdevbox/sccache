...
@SCDoc
(
	description = "Returns the TTL for the given object or 0 if not found",
	parameters =
	{
		"key", 	"Object's key"
	}
)
public class SCCommandGetObjectTTL implements SCCommand
{
	SCCommandGetObjectTTL()
	{
	}
	...
	public SCDataBuilder newBuilder()
	{
		return new SCDataBuilder()
		{
			...
			{
				long 		ttl = server.getTTL(fKey);
				connection.sendValue(Long.toString(ttl));
			}
