...
@SCDoc
(
	description = "Delete all objects associated with a group (a list of keys that were in the group is returned)",
	parameters =
	{
		"group spec",		"Group spec to delete"
	}
)
public class SCCommandRemoveGroup implements SCCommand
{
	SCCommandRemoveGroup()
	{
	}
	...
	public SCDataBuilder newBuilder()
	{
		return new SCDataBuilder()
		{
			...
			{
				List<String> keys = server.removeGroup(fGroup);
